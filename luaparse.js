...
((root, name, factory) => {
  ...
  function parseTableConstructor(flowContext) {
    ...
    while (true) {
      ...
      if (Punctuator === token.type && consume("[")) {
        ...
        features.setConstructors &&
        Punctuator === token.type &&
        consume(".")
      ) {
        value = parseExpectedExpression(flowContext);
        fields.push(finishNode(ast.tableSetValue(value)));
  ...
  const versionFeatures = {
    ...
    "FiveM5.4": {
      ...
      attributes: { const: true, close: true },
      relaxedUTF8: true,
      // FiveM's custom features
      inKeyword: true,
      compileTimeJenkins: true,
      compoundOperators: true,
      cStyleComments: true,
      safeNavigation: true,
      setConstructors: true,
    },
    LuaJIT: {
